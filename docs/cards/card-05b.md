...
范围：data/_dao_core.py（新）、tools/_query_analysis.py（新）、tools/_transfer_risk.py（新）、data/dao.py、tools/query.py、tools/transfer.py、tools/schemas.py、tools/_query_common.py、tests/test_dao.py、tests/test_query_analysis.py（新）、tests/test_transfer_risk.py（新）
要求：
...
2. 拆 `tools/_query_analysis.py`（收 query.py 的分析逻辑：`_baseline_series` / `_history_mean_cents` / 阈值检测等）与 `tools/_transfer_risk.py`（收 transfer.py 的 §5 风控/限额区 152–225 + §5 八个常量 + tier 计算 / new_payee 判定 / 降级因子等）。两者依赖 `tools/_query_common` **单向**，禁循环 import。
   - 另：transfer.py 的 Pydantic 模型（62–121，60 行）**归位 `tools/schemas.py`**（CLAUDE.md 本就要求模型放 schemas.py，卡 05 当时「冲突服从硬范围」暂放 transfer.py）；会话 helper（`_SESSION_ID`/`set_session_id`/`current_session_id`，12 行）**归 `tools/_query_common.py`**（与 `set_current_user` 同族）。这样 transfer.py 才拆到 ≤300。
