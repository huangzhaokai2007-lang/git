...
现有基础（2026-09-22，`734ac09`）：工具已有、聊天未接；L3 延迟、撤销与复核流程。

业务对应：`card_unlock` / `manage_card(unlock)`。
...
- 现有回归：[test_tools_card.py](../../tests/test_tools_card.py)、[test_tools_card_guards.py](../../tests/test_tools_card_guards.py)、[test_confirm_flow.py](../../tests/test_confirm_flow.py)
- 依赖：F01 + F04；用 F07 或测试夹具准备锁卡状态。

这些是定位入口，不意味着要改全部文件。实现仍落在原有五层目录；共享文件先与本周集成人员约定，新增契约或依赖先按项目规则审批。

## 本轮验收清单

- [ ] L3 双因子、延迟和人工复核标记按规格执行，不能直接即时解锁。
- [ ] 延迟期间可撤销，撤销后状态不变。
- [ ] 过期凭证、其他用户及重复提交受保护。
...
```bash
uv run pytest tests/test_tools_card.py tests/test_tools_card_guards.py tests/test_confirm_flow.py
```
