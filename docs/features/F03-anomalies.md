...
现有基础（2026-09-22，`734ac09`）：已有读路由；逐项核对异常规则和解释依据。

业务对应：`anomaly_check` / `detect_anomalies`。
...
- 现有回归：[test_query_analysis.py](../../tests/test_query_analysis.py)、[test_tools_query.py](../../tests/test_tools_query.py)
- 依赖：复用 F02 的流水数据口径。
...
```bash
uv run pytest tests/test_query_analysis.py tests/test_tools_query.py
```
