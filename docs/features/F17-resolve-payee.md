...
用户示例：“给李四转账，但联系人里有两个李四。”

现有基础（2026-09-22，`734ac09`）：转账内部已调用；同名反问、无匹配和脱敏。

业务对应：共用能力 / `resolve_payee`。
...
- 现有回归：[test_tools_transfer.py](../../tests/test_tools_transfer.py)、[test_confirm_flow.py](../../tests/test_confirm_flow.py)
- 依赖：沿用现有联系人库；F22 的新增路径已经可用。

这些是定位入口，不意味着要改全部文件。实现仍落在原有五层目录；共享文件先与本周集成人员约定，新增契约或依赖先按项目规则审批。

## 本轮验收清单

- [ ] 姓名和允许的手机号查询返回本人的脱敏候选。
- [ ] 同名必须追问，零匹配不随意选人。
- [ ] 作为转账共用能力复用，不新增未经批准的独立意图。
...
```bash
uv run pytest tests/test_tools_transfer.py tests/test_confirm_flow.py
```
