...
现有基础（2026-09-22，`734ac09`）：已有读路由；周期、状态和长期未扣费提示。

业务对应：`subscription_list` / `list_subscriptions`。
...
- 现有回归：[test_tools_subscription.py](../../tests/test_tools_subscription.py)、[test_subscription_wording.py](../../tests/test_subscription_wording.py)
- 依赖：既有订阅查询；为 F04/F18 提供目标和周期。

这些是定位入口，不意味着要改全部文件。实现仍落在原有五层目录；共享文件先与本周集成人员约定，新增契约或依赖先按项目规则审批。

## 本轮验收清单

- [ ] 展示脱敏用户范围内的商户、金额、周期和下次扣费日。
- [ ] 长期无扣费提示描述证据，不等同用户未使用。
- [ ] 状态筛选、无订阅和年费周期边界有用例。
...
```bash
uv run pytest tests/test_tools_subscription.py tests/test_subscription_wording.py
```
