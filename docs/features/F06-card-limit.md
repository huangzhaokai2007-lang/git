...
用户示例：“把这张信用卡额度调到我指定的金额。”

现有基础（2026-09-22，`734ac09`）：工具已有、聊天未接；区分授信额与单笔/单日限额。

业务对应：`card_limit_adjust` / `manage_card`。
...
- 现有回归：[test_tools_card.py](../../tests/test_tools_card.py)、[test_tools_card_guards.py](../../tests/test_tools_card_guards.py)
- 依赖：F01 + F04 公共写流程。
...
- [ ] 区分授信额度 adjust_limit 与交易限额 set_txn_limit。
- [ ] 预览展示原值、新值和档位，未确认不更改。
- [ ] 非法金额、非法卡状态、跨用户和重复请求受保护。
...
```bash
uv run pytest tests/test_tools_card.py tests/test_tools_card_guards.py
```
