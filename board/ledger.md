...
## 决策记录 2026-09-20 11:23（人类全批 SPEC-CHANGE 落地）

交付：
  - fc9d86a ：规格 §2 计数 15→16 + §5 删不可达档位金额分支(加注) + §7 容差收窄(裸数字仅精确匹配，带真值表)+ guard/facts_check.py 代码 + 16 条单测 + §8 用例格式追认；顺修 api_smoke/im-selftest 时段依赖。红线 **84→100**，单测 997→1013。
...
裁决（worker 6 条待拍板）：
  - ① #6 CLAUDE.md → **analyst 已亲自改**（含 .hermes.md 同类两处）
  - ② 追认越界 ⑥⑦（api_smoke.py + im/selftest.py 时段依赖修复，否则 verify 白天红）→ **追认**
  - ③ 红线 84→100 / 单测 997→1013 → README/答辩提纲 过期数字**派 worker 同步**
  - ④ 旧口径用例改判 1 条（"占比 0.32。" 由通过→越界）→ **接受**（#5 题中之义）
  - ⑤ api_smoke.main 42 行 → 并进本轮收尾小重构
