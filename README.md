...
- 30 条红队攻击集 + 970 条单测 + 30 条评测用例，**全部离线**：`bash scripts/verify.sh`
...
| 你说 | 它做了什么 | 回执（节选） |
| --- | --- | --- |
| 查一下余额 | `get_balance` | 您储蓄账户的余额是 **46,634.00** 元，可用余额也是 46,634.00 元（截至 2026-09-20T03:46:25） |
| 上个月花了多少 | `analyze_spending` | **2026-08** 一共支出 **9,152.00** 元，环比下降 **32%** |
| 我有哪些订阅？ | `list_subscriptions` | 目前共有 **4** 个订阅，其中 **1** 个疑似僵尸订阅 |
| 给王五转 100 元 | `preview_transfer` → 确认卡（L2） | 【转账确认卡】收款人：王五（137\*\*\*\*2003）；金额：**100.00** 元；权限档：L2（需输入短信验证码）；请回复「确认」继续 |
| （回复「确认」后输入验证码） | `execute_transfer`（幂等） | 已向王五转账 **100.00** 元，账户余额为 **46,534.00** 元（46,634.00 → 46,534.00） |
...
> 计数口径：`docs/01-接口规格.md` §2 标题写「15 个」，表内实际列出 T1–T16（T16 `plan_gift` 为后续加入的
> 跨场景联动）。本表照规格表列全；**计数差异请人类拍板**（改标题或改表，属规格变更）。
