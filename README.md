...
> 计数口径：规格 §2 标题与表内 **18 个**（T1–T16 + T17 `add_payee` + T18 `list_cards`）。
> ⚠️ **已知文档缺口（card-23 报备）**：卡 20 的 SPEC-CHANGE 只把 §2 标题改成 17，**规格 §2 表内从未补 T17 行**
> （本表与规格 §3 的意图清单里有 T17，规格 §2 表里没有）。规格只有人能改，补行待人类落盘 —— card-23 已把
> T18 行按授权补上（标题 → 18）。
