...
- 幂等：`preview_token` 是**一次性**的；执行后把结果快照写回 token，同 token 重复调用返回同一结果、
  ...
  线程级 TOCTOU）；**卡 14b-6**：token 快照落库 `idempotency`（`_load_token`/`_store_token` 薄壳），
  「先事务、后写快照」——扣款/流水/审计提交成功后才写 executed，重启后重放仍返回同结果，
  再叠加 DB 级条件更新守卫（受影响行 0 = 已被抢先执行）防多进程重复执行。
- 归属：收款人 / 账户必须属于当前用户（越权 → `FORBIDDEN`），会话用户沿用 `tools._query_common.set_current_user`。

模块分工（卡 05b 拆分后，依赖单向 `transfer → {_transfer_risk, _query_common, _transfer_token, schemas}`）：
- §5 权限档 / 降级因子 / 剩余限额 → `tools/_transfer_risk.py`（时间由参数注入）；
- 薄封装与会话上下文 → `tools/_query_common.py`；出入参模型 → `tools/schemas.py`；
- token 快照存储（`_TOKENS`/`_TOKENS_LOCK`/`_load_token`/`_store_token`）→ `tools/_transfer_token.py`（卡 14b-6）；
- 本模块只留 T6–T9 公开函数 + `_now`（测试用假时钟钉的就是它）。
...
from tools._transfer_token import (_TOKENS, _TOKENS_LOCK, _load_token, _load_token_authoritative,
                                   _raw_snapshot, _store_token)
...
def preview_transfer(payee_id: str, amount: int, schedule: str | None = None,
    ...
    token 登记落库 `idempotency`（卡 14b-6，重启后重放仍有效）、TTL 300 秒、绑定（payee_id + amount + 用户 + 档位）；本函数**不写任何流水与审计**。
    ...
    _store_token(token, {"user_id": current_user_id(), "session_id": current_session_id(),
...
def _execute_locked(preview_token: str, otp: str | None) -> ToolResult:
    ...
    token = _load_token_authoritative(preview_token)            # 卡 14b-6 MUST_FIX：state 判定只认库
    expect = _raw_snapshot(preview_token)                       # CAS 的 expected：加载时捕获的旧 json
    if token is None:
    ...
    if token["state"] == "executed":                            # 幂等：同 token 返回同一结果（重放不计数）
    ...
    try:                                                        # 预览之后余额/限额可能变化 → 执行前重校验
        ...
        if account["balance"] < amount + token["fee"]:
            return _fail(ErrorCode.INSUFFICIENT_FUNDS, "储蓄账户余额不足")
        tool_guard.check_write_rate(current_user_id(), tool="execute_transfer")   # 限流：先计数再校验，重放不计数
        conn = dao.connection()
        ...
        _rollback_token(token)
        return _dao_reject(exc)
    # 「先事务、后写快照」：扣款/流水/审计同一事务提交成功后才写 executed 快照，
    # 绝不出现「状态说 executed、库里却没有这笔流水」的假成功。
    executed = {**token, "state": "executed", "data": data.model_dump(), "facts": facts, "message": message}
    if not _store_token(preview_token, executed, expect_json=expect):     # DB 级守卫（真 CAS）：
        winner = _load_token_authoritative(preview_token)               # 受影响行 0 = 已被抢先执行 → 幂等返回赢家
        return _ok(winner["data"], winner["facts"], winner["message"])
    return _ok(executed["data"], executed["facts"], executed["message"])

def _rollback_token(token: dict) -> None:
    """「先事务后写快照」后：事务失败时根本没写 executed 快照，这里只清理内存半成品状态。"""
    token["state"] = "preview"
    token.pop("data", None)
    token.pop("facts", None)
    token.pop("message", None)
